...
data_transforms = {
    'train':
        transforms.Compose([
        transforms.Resize([96, 96]),#定义为统一大小
        transforms.RandomRotation(45),#随机旋转，-45到45度之间随机选
        transforms.CenterCrop(64),#从中心开始裁剪
        transforms.RandomHorizontalFlip(p=0.5),#随机水平翻转 选择一个概率
        transforms.RandomVerticalFlip(p=0.5),#随机垂直翻转
        transforms.ColorJitter(brightness=0.2, contrast=0.1, saturation=0.1, hue=0.1),#参数1为亮度，参数2为对比度，参数3为饱和度，参数4为色相 极端场景再详细改
        transforms.RandomGrayscale(p=0.025),#概率转换成灰度率，3通道就是R=G=B 一般不进行改变
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])#均值，标准差。用大数据集已经得出的结果
    ]),
    'valid':
        transforms.Compose([
...
#是否用训练好的特征来做
feature_extract = True #都用训练好的特征，先不更新
...
# 优化器设置
optimizer_ft = optim.Adam(params_to_update, lr=1e-2)#要训练什么参数
...
#model_ft, val_acc_history, train_acc_history, valid_losses, train_losses, LRs  = train_model(model_ft, dataloaders, criterion, optimizer, num_epochs=50)
